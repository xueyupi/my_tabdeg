...
parser.add_argument('-output', metavar='OUTPUT',
                    help='Input folder', default=r"D:\PyCharmProjects\pythonProject\TAB_DEGNET\COAD")
parser.add_argument('-batch-size', type=int, default=512,        # 256
                    help='Batch size')
...
for col in (NORMAL + TUMOR):
    ...
    if IS_TRAIN:
        ...
        # 95 > 90> 50 > 80 # KIRC的
        transformer = QuantileTransformer(n_quantiles=95, random_state=0, output_distribution="normal")
...
data = pd.concat([pd.DataFrame(train_features[NORMAL]), pd.DataFrame(test_features[NORMAL])])
if IS_TRAIN:
    fa = FactorAnalysis(n_components=n_comp, random_state=512).fit(data[NORMAL])    #256<42
...
if IS_TRAIN:
    fa = FactorAnalysis(n_components=n_comp, random_state=512).fit(data[TUMOR])  #256<42
...
def fe_cluster(train, test, n_clusters_normal = 35, n_clusters_tumor = 5, SEED = 123):  # 123
...
# pd.to_pickle(train, f"{INT_DIR}/{NB}_train.pkl")
# pd.to_pickle(test, f"{INT_DIR}/{NB}_test.pkl")

train.to_pickle(f"{INT_DIR}/{NB}_pre_train7.pkl")
test.to_pickle(f"{INT_DIR}/{NB}_pre_test7.pkl")
